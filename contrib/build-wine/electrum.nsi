...
;Interface Settings
  ...
  !define MUI_ICON "tmp\electrum-btcp\icons\electrum.ico"
...
Section
  ...
  ;Links bitcoinprivate: URI's to Electrum-BTCP
  ...
  WriteRegStr HKCU "Software\Classes\bitcoinprivate" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\bitcoinprivate\shell\open\command" "" "$\"$INSTDIR\electrum-btcp-${PRODUCT_VERSION}.exe$\" $\"%1$\""
